...
ClassImp(TaEigenLagrangian);

TaEigenLagrangian::TaEigenLagrangian(Int_t ana_index,TaConfig *aConfig){
#ifdef NOISY
...
TMatrixD TaEigenLagrangian::GetDetMonCovMatrix(Int_t imini){
...
void TaEigenLagrangian::LoadConstraint(Int_t ana_index, TaConfig *aConfig){
#ifdef NOISY
  cout << __PRETTY_FUNCTION__ << endl;
#endif
...
vector<vector<Double_t> >  TaEigenLagrangian::Solve(TMatrixD CovDM, TMatrixD CovMM){
#ifdef NOISY
  cout << __PRETTY_FUNCTION__ << endl;
#endif
...
TString TaEigenLagrangian::GetBaseName(TString in){
  in.ReplaceAll("diff_","");
  in.ReplaceAll("asym_","");
  return in;
}

Int_t TaEigenLagrangian::FindRawDVIndexFromList(TString raw_name){
  Int_t index = -1;
...
void TaEigenLagrangian::LoadRawDVList(vector<TString> fRawElementList){
  auto iter_ele = fRawElementList.begin();
  while(iter_ele!=fRawElementList.end()){
...
void TaEigenLagrangian::LoadRawDVList(TString raw_name){

...
def main():
    ...
    lastrun=0
    _runlist=[]
    runfile=open(_source+"/prex-runlist/simple_list/all_crex.list","r")
    for line in runfile:
    ...
    _workflowID="Lagrange_SAM_"+str(firstrun)+"_"+str(lastrun)
...
def createXMLfile(mssdir,source,rootout,nStart,nStop,email,workflowID,runlist):
    ...
    for nr in runlist:
        ...
        f.write("    cd "+source+"\n")
        f.write("    "+source+"/sam-respin1-scripts/sam_crex.sh "+str(nr)+"; \n")
        f.write("    ]]></Command>\n")
        f.write("    <Stdout dest=\""+source+"/LogFiles/sam_respin1_log"+"_%04d"%(nr)+".out\"/>\n")
        f.write("    <Stderr dest=\""+source+"/LogFiles/sam_respin1_log"+"_%04d"%(nr)+".err\"/>\n")
        f.write("  </Job>\n\n")

...
void SolveMergedCycles_5Coils(Int_t slug_id, Bool_t kMatrixOutput=kFALSE);
void SolveMergedCycles_5Coils(){
  for(int i=1;i<=94;i++)
    SolveMergedCycles_5Coils(i,kTRUE);
}
void SolveMergedCycles_5Coils(Int_t slug_id,Bool_t kMatrixOutput){
  ...
  coil_index = required_index; // FIXME: dirty trick 
  Int_t nCoil = coil_index.size();
  ...
  // ++++++++++ Writing TTree
  TString rootfile_name = Form("./slopes/slug%d_dit_slope_merged_cycle_5coils.root",slug_id);
  ...
  gSystem->Exec(Form("pdfunite `ls ./plots/slug%d_dit_slope_buff_*.pdf` ./plots/slug%d_dit_slope_5coils_avg.pdf",
		     slug_id,slug_id));
  ...
  if(kMatrixOutput){
	...
	for(int imon=0;imon<nMon;imon++)
      ...
      TString out_filename = Form("./matrices/prex_5coils_avg_slope_matrix.%s.root",
				  range_tag.Data());

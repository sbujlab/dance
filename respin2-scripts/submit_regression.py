...
def main():
    ...
    _workflowID="NewRegression_"+str(firstrun)+"_"+str(lastrun)
...
def createXMLfile(mssdir,source,rootout,nStart,nStop,email,workflowID,runlist):
    ...
    for nr in runlist:
        ...
        f.write("    cd "+source+"\n")
        f.write("    "+source+"/regression.sh "+str(nr)+"; \n")
        f.write("    ]]></Command>\n")
        f.write("    <Stdout dest=\""+source+"/LogFiles/regression_ifarmlog"+"_%04d"%(nr)+".out\"/>\n")
        f.write("    <Stderr dest=\""+source+"/LogFiles/regression_ifarmlog"+"_%04d"%(nr)+".err\"/>\n")
        f.write("  </Job>\n\n")

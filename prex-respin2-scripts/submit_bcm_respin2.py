...
def main():
    ...
    _workflowID="bcm_"+str(firstrun)+"_"+str(lastrun)

    createXMLfile(_mssdir,_source,_rootout,_nrStart,_nrStop,_email,_workflowID,_runlist)

    if submit==1:
        if useSWIF==1:
            print "Submitting BCM prompt analysis for runs "+str(firstrun)+" to "+str(lastrun)+" using designated SWIF workflow "+str(_workflowID)
...
def createXMLfile(mssdir,source,rootout,nStart,nStop,email,workflowID,runlist):
    ...
    for nr in runlist:
        ...
        f.write("    cd "+source+"\n")
        f.write("    "+source+"/bcm_respin2.sh "+str(nr)+"; \n")
        f.write("    ]]></Command>\n")
        f.write("    <Stdout dest=\""+source+"/LogFiles/bcm_respin2log"+"_%04d"%(nr)+".out\"/>\n")
        f.write("    <Stderr dest=\""+source+"/LogFiles/bcm_respin2log"+"_%04d"%(nr)+".err\"/>\n")
        f.write("  </Job>\n\n")

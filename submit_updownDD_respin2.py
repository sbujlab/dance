...
def main():
    ...
    _workflowID="updownDD_"+str(firstrun)+"_"+str(lastrun)
...
def createXMLfile(mssdir,source,rootout,nStart,nStop,email,workflowID,runlist):
    ...
    for nr in runlist:
        ...
        f.write("    cd "+source+"\n")
        f.write("    "+source+"/updownDD_respin2.sh "+str(nr)+"; \n")
        f.write("    ]]></Command>\n")
        f.write("    <Stdout dest=\""+source+"/LogFiles/updownDD_respin2log"+"_%04d"%(nr)+".out\"/>\n")
        f.write("    <Stderr dest=\""+source+"/LogFiles/updownDD_respin2log"+"_%04d"%(nr)+".err\"/>\n")
        f.write("  </Job>\n\n")
